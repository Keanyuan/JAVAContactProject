<template>
	<view class="main">
		...
		<view class="check-food">
			<data-select-item
				:selected="selectedCan1.selected"
				:title="selectedCan1.cal"
				@normalClick="normalClick(1)"
				@selectClick="selectClick(1)"
				@cancleClick="cancleClick(1)"
			></data-select-item>
			<data-select-item
				:selected="selectedCan2.selected"
				:title="selectedCan2.cal"
				@normalClick="normalClick(2)"
				@selectClick="selectClick(2)"
				@cancleClick="cancleClick(2)"
			></data-select-item>
			<data-select-item
				:selected="selectedCan3.selected"
				:title="selectedCan3.cal"
				@normalClick="normalClick(3)"
				@selectClick="selectClick(3)"
				@cancleClick="cancleClick(3)"
			></data-select-item>
		</view>
...
export default {
	...
	methods: {
		...
		checkLocalInfo() {
			...
			// 处理排序 list 按时间排序
			health_locals.sort(function(a, b) {
			...
			// 按 早中晚餐排序
			for(let i in health_locals){
				health_locals[i].canItems.sort((a, b) => a.canId - b.canId);
			}
				...
				for (let var1 in this.healthDateList) {
					if (this.healthDateList[var1].date == this.selectDate) {
						...
						if (dataItem.canId1) {
							...
							this.selectedCan1.cal = dataItem.canId1.calTotal + '';
						} else {
						...
						if (dataItem.canId2) {
							...
							this.selectedCan2.cal = dataItem.canId2.calTotal + '';
						} else {
						...
						if (dataItem.canId3) {
							...
							this.selectedCan3.cal = dataItem.canId3.calTotal + '';
						} else {
